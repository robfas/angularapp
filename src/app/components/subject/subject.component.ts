...
export class SubjectComponent implements OnInit {
  ...
  saveSubject(idtypeSubject, subjdescr, subjcfu, idteacher){
    ...
    this.subjectofstudyService.saveSubject({name: this.typeSubjects[0].name, description: subjdescr, teacherDTO: this.teachers[0], cfu: subjcfu, typeSubjectDTO: this.typeSubject} as SubjectStudy).subscribe(subject => {
      console.log(subject);
    });
  }
}
